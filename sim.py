...
class SimParams:
    def __init__(self):
        ...
        types = ['multi-tone', 'continuous_wave', 'pulsed', 'bandlimited_gaussian', 'pseudocode']
        ...
        self.Tc = 0.9775e-6  # 伪码码元宽度(s)，C/A码典型值, P(Y)码 9.7752e-5 (s)，M码为1e-4 (s)
...
def calc_power_spectral_density(f, params, cnt):
    ...
    if jam_type_ == "continuous_wave":
        ...
        # 伪码干扰：使用伪随机码
        GJ = 1 if abs(f - params.fc) < params.jam_bandwidth/2 else 0
...
def calc_cnr(Pj_list, params):
    ...
    for p_j in Pj_list:
        ...
        integral_total = integral_total + Pj_list[cnt] * integral_result
        cnt += 1
...
# ========================= 5. 跟踪环误差计算（PLL+DLL）=========================
def calc_tracking_errors(C_NJ_dB, params, Pj_list):
    """
    ...
    return sigma_jpll, sigma_jdll_total
...
# ========================= 7. 定位误差求解（分失锁/未失锁场景）=========================
def solve_position_error(target_pos, satellite_pos, Pj_list, params):
    """
    分场景计算定位误差
    ...
    # 1. 计算载噪比
    C_NJ_dB = calc_cnr(Pj_list, params)
    if C_NJ_dB < 30:  # 干扰有效（载噪比低于正常阈值）
        # 2. 计算跟踪环误差
        sigma_jpll, sigma_jdll = calc_tracking_errors(C_NJ_dB, params, Pj_list)
...
def main_simulation():
    ...
    for t in range(time_steps):
        ...
        pos_error, unlock_flag, cnr = solve_position_error(target_pos, params.satellite_pos, Pj_list, params)
